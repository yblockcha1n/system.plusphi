...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        // ダークでは 10% の黒を敷いても背景と区別が付かないので濃くする
...
/**
 * 左右から出すシートの幅。data-side で絞っているのは、上下から出すシートは
 * inset-x-0 で横幅いっぱいに広げるため、ここの max-w が邪魔になるから。
 */
const SIDE_WIDTH = [
  "sm:data-[side=left]:w-1/3 sm:data-[side=left]:min-w-96 sm:data-[side=left]:max-w-2xl",
  "sm:data-[side=right]:w-1/3 sm:data-[side=right]:min-w-96 sm:data-[side=right]:max-w-2xl",
].join(" ")
    ...
    <SheetPortal>
      ...
      <SheetPrimitive.Popup
        ...
        className={cn(
          ...
          // 横から出すシートの幅。
          //  - モバイル(〜639px): 画面いっぱい。狭い画面で脇に本文を残しても読めない。
          //  - それ以上        : 画面幅の 1/3。ただし極端に狭く/広くならないよう
          //                      24rem〜42rem に収める。
          // 幅は各画面ではなくここで一元管理する（画面ごとに指定するとバラつくため）。
          SIDE_WIDTH,
          className
        )}
        {...props}
